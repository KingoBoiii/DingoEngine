...
// All Jolt usage is confined to this .cpp (+ the engine-internal Physics3DData.h).
#include "DingoEngine/Physics/Physics3DData.h"
...
namespace Dingo
{
	...
	PhysicsWorld3D* PhysicsWorld3D::Create(const PhysicsWorld3DParams& params)
	{
		AcquireJolt();

		PhysicsWorld3D* world = new PhysicsWorld3D();
		world->m_Data = new Internal::Physics3DData(params.MaxBodies);
		world->m_Data->PhysicsSystem.SetGravity(ToJolt(params.Gravity));
		return world;
	}
	...
	PhysicsBodyId3D PhysicsWorld3D::CreateBody(const RigidBodyParams3D& params)
	{
	...
	void PhysicsWorld3D::DestroyBody(PhysicsBodyId3D body)
	{
	...
	bool PhysicsWorld3D::IsBodyValid(PhysicsBodyId3D body) const
	{
		if (body == k_InvalidBody3D)
			return false;
	...
	void PhysicsWorld3D::Step(float deltaTime, int collisionSteps)
	{
		if (deltaTime <= 0.0f)
			return;
	...
	glm::vec3 PhysicsWorld3D::GetPosition(PhysicsBodyId3D body) const
	{
		if (body == k_InvalidBody3D)
	...
	glm::quat PhysicsWorld3D::GetRotation(PhysicsBodyId3D body) const
	{
		if (body == k_InvalidBody3D)
	...
	glm::mat4 PhysicsWorld3D::GetTransform(PhysicsBodyId3D body) const
	{
		return glm::translate(glm::mat4(1.0f), GetPosition(body)) * glm::mat4_cast(GetRotation(body));
	}

	void PhysicsWorld3D::SetLinearVelocity(PhysicsBodyId3D body, const glm::vec3& velocity)
	{
		if (body == k_InvalidBody3D)
			return;
	...
	glm::vec3 PhysicsWorld3D::GetLinearVelocity(PhysicsBodyId3D body) const
	{
		if (body == k_InvalidBody3D)
			return glm::vec3(0.0f);
	...
	void PhysicsWorld3D::ApplyImpulse(PhysicsBodyId3D body, const glm::vec3& impulse)
	{
		if (body == k_InvalidBody3D)
			return;
	...
	void PhysicsWorld3D::ApplyForce(PhysicsBodyId3D body, const glm::vec3& force)
	{
		if (body == k_InvalidBody3D)
			return;
